...
describe("POST /submit", () => {
    ...
    describe("Test cases with mocked fetching", () => {
        ...
        it("should respond with status code 400 with invalid token and input secret", async () => {
            fetch.mockReturnValue(Promise.resolve({ json: () => Promise.resolve({"success": false, "error-codes": ["invalid-input-response", "invalid-input-secret"]}) }));
            const response = await supertest(verifyRecaptcha).post("/submit").send({ captcha: "token" });
        ...
        it("should respond with status code 400 with invalid token", async () => {
            fetch.mockReturnValue(Promise.resolve({ json: () => Promise.resolve({"success": false, "error-codes": ["invalid-input-response"]}) }));
            const response = await supertest(verifyRecaptcha).post("/submit").send({ captcha: "token" });
        ...
        it("should respond with status code 401 with invalid secret", async () => {
            fetch.mockReturnValue(Promise.resolve({ json: () => Promise.resolve({"success": false, "error-codes": ["invalid-input-secret"]}) }));
            const response = await supertest(verifyRecaptcha).post("/submit").send({ captcha: "token" });
        ...
        it("should respond with status code 400 with timeout or duplicate", async () => {
            fetch.mockReturnValue(Promise.resolve({ json: () => Promise.resolve({"success": false, "error-codes": ["timeout-or-duplicate"]}) }));
            const response = await supertest(verifyRecaptcha).post("/submit").send({ captcha: "token" });
