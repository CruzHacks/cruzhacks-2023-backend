...
describe("Application Test", () => {
  ...
  describe("Given submit invalid form data", () => {
    ...
    it("Should return 400 and proper error-codes given email > 100chars", async () => {
      ...
      const res = await request(application).post("/submit").field("email", "oooooooooonnnnnnnnnneeeeeeeeeehhhhhhhhhhuuuuuuuuuunnnnnnnnnnddddddddddrrrrrrrrrreeeeeeeeeeddddddddddchars@email.com");
      expect(jwtCheck).toHaveBeenCalledTimes(1);
      expect(hasUpdateApp).toHaveBeenCalledTimes(1);
      expect(res.status).toBe(400);
      expect(res.body.message).toBe("Form Validation Failed");
    ...
    it("Should return 400 and proper error-codes given empty first and last name", async () => {
      ...
      const res = await request(application).post("/submit").field("email", "user@example.com").field("fname", "").field("lname", "");
      expect(jwtCheck).toHaveBeenCalledTimes(1);
      expect(hasUpdateApp).toHaveBeenCalledTimes(1);
      expect(res.status).toBe(400);
      expect(res.body.message).toBe("Form Validation Failed");
    ...
    it("Should return 400 and proper error-codes given first and last name too long", async () => {
      ...
      const res = await request(application).post("/submit").field("email", "user@example.com").field("fname", "JohnPaulJacobTheThirdFourthAndFifthButSometimesTheSixth").field("lname", "PeabodyPearcexxxxxxxxxxxxxxxxx");
      expect(jwtCheck).toHaveBeenCalledTimes(1);
      expect(hasUpdateApp).toHaveBeenCalledTimes(1);
      expect(res.status).toBe(400);
      expect(res.body.message).toBe("Form Validation Failed");
    ...
    it("Should return 400 and proper error-codes given first and last name valid but other invalid fields", async () => {
      ...
      const res = await request(application).post("/submit").field("email", "user@example.com").field("fname", "Jacob").field("lname", "Jacobi");
      expect(jwtCheck).toHaveBeenCalledTimes(1);
      expect(hasUpdateApp).toHaveBeenCalledTimes(1);
      expect(res.status).toBe(400);
      expect(res.body.message).toBe("Form Validation Failed");
    ...
    it("Should return 400 and proper error-codes given empty phone number", async () => {
      ...
      const res = await request(application).post("/submit").field("email", "user@example.com").field("fname", "Jacob").field("lname", "Jacobi").field("phone", "");
      expect(jwtCheck).toHaveBeenCalledTimes(1);
      expect(hasUpdateApp).toHaveBeenCalledTimes(1);
      expect(res.status).toBe(400);
      expect(res.body.message).toBe("Form Validation Failed");
    ...
    it("Should return 400 and proper error-codes given phone number too long", async () => {
      ...
      const res = await request(application).post("/submit").field("email", "user@example.com").field("fname", "Jacob").field("lname", "Jacobi").field("phone", "+1 (925)-111-1111");
      expect(jwtCheck).toHaveBeenCalledTimes(1);
      expect(hasUpdateApp).toHaveBeenCalledTimes(1);
      expect(res.status).toBe(400);
      expect(res.body.message).toBe("Form Validation Failed");
    ...
    it("Should return 400 and proper error-codes given age too low", async () => {
      ...
      const res = await request(application).post("/submit").field("email", "user@example.com").field("fname", "Jacob").field("lname", "Jacobi").field("phone", "925-111-1111").field("age", 3);
      expect(jwtCheck).toHaveBeenCalledTimes(1);
      expect(hasUpdateApp).toHaveBeenCalledTimes(1);
      expect(res.status).toBe(400);
      expect(res.body.message).toBe("Form Validation Failed");
